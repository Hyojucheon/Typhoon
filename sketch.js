...
const TEXT_SIZE_INFO_TITLE = 40;
const TEXT_SIZE_INFO = 20;
  ...
  if (capture.pixels.length > 0) {
    ...
    if (flow.flow && flow.flow.u != 0 && flow.flow.v != 0) {
      ...
      flow.flow.zones.forEach(function(zone) {
        stroke(map(zone.u, -step * 3, +step * 3, 0, 255),
          map(zone.v, -step * 3, +step * 3, 0, 255), 128);
        line(zone.x * 3, zone.y * 3, zone.x * 3 + zone.u * 3, zone.y * 3 + zone.v * 3);
      })
    }
